...
public class CommandLeave implements CommandExecutor {
    ...
    public boolean onCommand(@NonNull CommandSender sender, @NonNull Command command, @NonNull String label, @NonNull String[] args) {
        ...
        if (game.getPlayerManager().leavePlayer(player)) {
            Bukkit.broadcastMessage(String.format("§e%s §fпокинул игру", sender.getName()));
        }
        else {
            sender.sendMessage("§cВы не участник игры");
        }
        return true;
    }
}
